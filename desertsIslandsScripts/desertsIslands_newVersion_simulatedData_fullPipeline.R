...
print("Deserts done")
print(dim(desertDF))
write.table(desertDF, paste(popName, "_minorParentDeserts_AIMs_cMwins_raw_", repNum, ".txt", sep=""), quote=F, row.names = F, sep="\t")
...
print("Islands done")
print(dim(islandDF))
write.table(islandDF, paste(popName, "_minorParentIslands_AIMs_cMwins_raw_", repNum, ".txt", sep=""), quote=F, row.names = F, sep="\t")
...
mergeDeserts <- toKeepSNPs
write.table(toKeepSNPs, paste(popName, "_minorParentDeserts_AIMs_cMwins_merged_", repNum, ".txt", sep=""), row.names=F, quote=F, sep="\t")
print(popName)
print("Deserts")
print(dim(regions))
print(dim(mergeDF))
...
toKeepAIMs <- subset(toKeep, nAIMs>=10)
toKeepSNPs <- subset(toKeepAIMs, cMsnps>=10)
write.table(toKeepSNPs, paste(popName, "_minorParentIslands_AIMs_cMwins_merged_", repNum, ".txt", sep=""), row.names=F, quote=F, sep="\t")
print(popName)
print("Islands")
print(dim(regions))
print(dim(mergeDF))
...
selectedRegs$pass2.5 <- NA
selectedRegs$pass5 <- NA
for (i in 1:nrow(selectedRegs)) {
  ...
  selectedRegs$pass2.5[i] <- closestAIM$minPar<=quant2.5
  selectedRegs$pass5[i] <- closestAIM$minPar<=quant5
}
